...
**Phase F-4 role-management writers shipped 2026-05-05** (this session):
- ✅ `changeRole(orgId, byUserId, targetUserId, newRole)` — flag-gated, transactional. Validates `newRole ∈ {admin, member}` (cannot promote to owner via this path; must use `transferOwnership`). Rejects self-targeting. Pulls actor + target rows in one query, then enforces TWO permission predicates: **strict outrank** (`actorRank <= targetRank` → reject; admins cannot demote owners or peer admins) AND **authority-to-grant** (`actorRank < newRoleRank` → reject; members can't grant admin even if the type-system doesn't catch it through some future refactor).
- ✅ `removeMember(orgId, byUserId, targetUserId)` — flag-gated, transactional. Owner-protection invariant: `target.role === "owner"` → reject with "transfer ownership first" copy (prevents accidentally orphaning an org). Self-leave allowed for non-owners (members and admins can resign). Cross-user removal requires strict-outrank (`actorRank <= targetRank` → reject). DELETE returns affected-rows count for honest "not found" vs "did remove" branching.
- ✅ `transferOwnership(orgId, fromUserId, toUserId)` — flag-gated, 3-write atomic transaction. Verifies `organizations.owner_user_id` column matches `fromUserId` (column-not-role check — cheaper + the source of truth, since `organization_members.role='owner'` is meant to be redundant and could drift on writer bugs). Rejects self-transfer. Requires `toUserId` already a member (no auto-invite — the new owner must already be a recognized account on the org). Three writes: `organizations.owner_user_id` swap + former-owner row demoted to `admin` (NOT removed — they keep org access by default; an explicit `removeMember` is a separate step) + new-owner row promoted to `owner`. Whole thing inside `db.transaction` so a partial-state failure rolls back.
...
**Phase F-4 settings page shipped 2026-05-06** (this session):
- ✅ `lib/orgs/writers.ts:renameOrg(orgId, byUserId, newName)` — owner-only via column check on `organizations.owner_user_id`. 255-char name validation matches column width. Slug intentionally NOT regenerated so existing `/app/org/<slug>` URLs (bookmarks, OAuth callback registrations, shared links) keep working — slug regen would be a separate writer with its own breakage story.
- ✅ `lib/orgs/writers.ts:deleteOrg(orgId, byUserId)` — owner-only via column check, 3-write atomic cascade (invites → members → org) inside one transaction. Honest cascade scope: `ai_usage` / `credit_ledger` / `files` / `ai_outputs` are scoped to `user_id` (not `organization_id`) so they survive deletion — users keep their personal usage history.
- ✅ `/app/org/[slug]/settings/page.tsx` — owner-only render. `getMemberRole !== "owner"` → `notFound()` (anti-existence-leak — admins + members don't even know the settings URL exists). Two sections: Rename (top) + "Danger zone — delete organization" (bottom, with red-tint border).
- ✅ `RenameOrgForm.tsx` — Save button disabled when name unchanged (avoids no-op writes). Inline success/error states.
- ✅ `DeleteOrgForm.tsx` — two-step confirmation. Step 1: collapsed "Delete organization" button. Step 2: typed-name confirmation field; submit disabled until typed name matches org name exactly. On success, `router.push("/app/dashboard")` since the org URL is now a 404.
- ✅ `renameOrgAction` + `deleteOrgAction` — outer-layer `getMemberRole === "owner"` check + writer ALSO re-checks column at tx layer (defense-in-depth, defends against stale tab from before a transferOwnership).
- ✅ `deleteOrgAction` re-checks `confirmName === expectedName` at the action layer — hostile clients can skip the form's typed-name gate, so the action is the source of truth.
- ✅ Org-landing page renders "Settings →" link in the footer ONLY when `role === "owner"`. Admins + members don't see the link (the page would `notFound()` on them anyway, but hiding the link keeps the surface honest).
- 33-assertion guard expansion (Section M): writer exports + owner column-check at tx layer + 255-char name validation + slug-stability invariant + cascade dependency order (invites → members → org), settings page existence + auth gate + callbackUrl preservation + owner-only `notFound()` + form imports, settings actions module shape + anti-impersonation positive (`byUserId: userId`) + negative (`!byUserId: input.byUserId`) + owner-check + typed-name confirmation re-check at action layer + OrgWriteError mapping, RenameOrgForm + DeleteOrgForm shape (use-client, action calls, two-step armed-pattern, post-success navigation), org-landing page Settings-link owner-only gate.
...
- Change-billing-mode action (today the field is set at create-org time and never changes; Phase F-4's billing wire-up will need this paired with the credit_ledger routing rework)
