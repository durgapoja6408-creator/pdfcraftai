...
### From 2026-04-20 production readiness audit

- [ ] **(SEV-1, security) CSP is being stripped by LiteSpeed/LSAPI in production.** `curl -sI https://pdfcraftai.com/` returns only `content-security-policy: upgrade-insecure-requests` — the full multi-directive policy defined in `next.config.mjs` is gone. `.htaccess` inspected via SSH has no CSP override, so the strip is at the LSAPI layer. **Fix:** add `Header always set Content-Security-Policy "..."` to `.htaccess` on the origin (SSH + edit). Full directive string is in `PRODUCTION_AUDIT_2026-04-20.md` §3 SEV-1. Verify with `curl -sI https://pdfcraftai.com/ \| grep content-security-policy` post-change. This restores the PCI SAQ-A scope lockdown the `next.config.mjs` comments promise.
- [ ] **(SEV-2, SEO/social) Per-page `openGraph` + `twitter` blocks not overriding layout defaults.** `/about`, `/tool/merge`, etc. ship the homepage's generic og:title / og:description on social shares. `<title>` and `<meta description>` are correctly per-page, so search SEO is fine — this only affects share cards. **Fix:** add `openGraph: { title, description }` + `twitter: { title, description }` to each page's `metadata` export (About, every Tool/* via `generateMetadata`, each blog post via `[slug]/page.tsx`, Pricing, Enterprise, Contact, Help, Security, Privacy, Terms, DPA, Cookies, Refund-Policy, Status, Sitemap-Page). Alternative: use `title.template` in root layout so defaults propagate.
- [ ] **(SEV-3, SEO) `/` is missing `<link rel="canonical">`.** Every other page has one. Add `alternates: { canonical: "/" }` to `app/page.tsx` metadata.
- [ ] **(SEV-3, consistency) `/api` uses `twitter:card=summary`** while every other route uses `summary_large_image`. Standardize in `app/api/page.tsx`.
