...
| `PROD_E2E_TEST_EMAIL` | GitHub Actions secret: `durgapoja6408@gmail.com` | user_id `6b303c3b-ddfd-48fc-9162-2556d077fece`. MUST stay a NON-admin, non-`rajasekarjavaee` account — `ADMIN_EMAILS=rajasekarjavaee@gmail.com` + Gmail +alias normalization make every `rajasekarjavaee+*@gmail.com` read as admin, which breaks the "non-admin blocked from /admin" SEV-0 test. KEEP durgapoja6408 (do NOT delete) as the dedicated non-admin test identity. |
| `PROD_E2E_TEST_PASSWORD` | `Cognizant@2020` (bcrypt-verified 2026-06-03) | User-side only (bcrypt hash; not recoverable from DB without rotation) |
...
3. **Once SSH works**, fetch all the prod-env secrets in one shot:
   ...
   ```
4. **Test account password** — the prod-e2e account is `durgapoja6408@gmail.com` / `Cognizant@2020` (the dedicated NON-admin test identity — must NOT be a rajasekarjavaee gmail alias, which would read as admin). Re-paste it, or rotate via the prod "Forgot password?" flow.
...
- **CSP IS NOW SERVED BY A CLOUDFLARE TRANSFORM RULE (2026-06-03), which supersedes the `.htaccess` approach above for CSP specifically.** Root cause re-confirmed: the Hostinger LiteSpeed/Passenger origin collapses the multi-directive CSP that Next.js emits down to its LAST directive (`upgrade-insecure-requests`) — verified at the origin IP directly (so it is the origin, NOT Cloudflare). The Passenger-generated `.htaccess` carries no CSP and is regenerated on deploy, so any earlier `.htaccess` CSP override did NOT survive. FIX: a Cloudflare **Response Header Transform Rule** named **"Full CSP header (all responses)"** (zone `pdfcraftai.com`, account `b008d9e115edfcf4c560862bf4c73393`) does **Set static** `Content-Security-Policy` = the full policy, OVERWRITING the origin's stripped header at the edge. Verified live: `curl -sI https://pdfcraftai.com/ | grep -i content-security-policy` returns the full ~1044-char policy incl. `challenges.cloudflare.com`. **DRIFT WARNING: the CSP value now lives in TWO places — `next.config.mjs` (canonical source) AND the Cloudflare rule. If you change the CSP in code you MUST update the Cloudflare rule too (Rules -> Transform Rules -> "Full CSP header (all responses)"), or prod keeps serving the old policy.** Resolve the exact string by evaluating the CSP array in `next.config.mjs`. Single-value headers (X-Frame-Options, HSTS) pass through LiteSpeed fine and are unaffected.

- **Windows workspace folder auto-syncs from GitHub (2026-06-03).** The user's Windows folder is a real git repo tracking `origin/main` (re-initialized Windows-side on 2026-06-03 because the mount's `.git` is unusable through FUSE). A Windows **Task Scheduler** job **`pdfcraftai-Auto-Sync`** runs `.claude/autosync-pull.ps1` every 15 min: `git fetch` + `git merge --ff-only origin/main` (pull-only, never pushes; logs to `.claude/autosync.log`). Model: **Cowork edits the sandbox clone -> pushes to GitHub -> the Windows folder auto-pulls within 15 min**, so you no longer need to rsync to the mount after a push. (The sandbox still cannot use the mount's `.git` — keep the real `.git` in the sandbox clone as before.)

- **E2E TESTING KNOBS — TEMPORARY; keep OFF/at-default in normal operation (2026-06-03).** To let the prod-e2e suite exercise the gated flows (signup, real Razorpay checkout) these knobs exist; they WEAKEN protection while set, so use a short window and revert promptly:
  - `E2E_CHECKOUT_GEO_OVERRIDE_USER_ID` (Hostinger env) — when set to a user_id, `createCheckoutAction` forces route country = `IN` for ONLY that user, so the dedicated test account can reach Razorpay test-mode checkout from a non-IN CI runner. Unset = fully disabled; scoped to a single id; logs an `e2e_geo_override` event. Set to `6b303c3b-ddfd-48fc-9162-2556d077fece` (durgapoja6408) for a test window, UNSET after. (Code: `lib/payments/checkout-actions.ts`.)
  - Turnstile **test keys** (always-pass): `NEXT_PUBLIC_TURNSTILE_SITE_KEY=1x00000000000000000000AA`, `TURNSTILE_SECRET_KEY=1x0000000000000000000000000000000AA` — lets automated signup through. Swap the real keys back after. (Do NOT just unset the secret — it fails CLOSED in prod and blocks signup.)
  - Rate-limit relaxers: `MAX_SIGNUPS_PER_BUCKET`, `USER_DAILY_COST_MICROS_CAP`, `LOGIN_MAX_FAILURES` — raise during testing, revert after.
- **Comprehensive prod crawl** — `scripts/crawl-prod.mjs` + `.github/workflows/crawl.yml` (workflow_dispatch). Playwright/Chromium crawl of EVERY `sitemap.xml` URL: scroll-to-bottom, console errors, page exceptions, failed (>=400) sub-requests, broken images; uploads `crawl-report.json`. Runs in **GitHub Actions** because Chromium **cannot launch in the Cowork sandbox** (SIGSEGV — confirmed 2026-06-03; the sandbox has no kernel/seccomp surface for a browser). Run: `gh workflow run crawl.yml -f max=0` (0 = all).
